...
export default function SkillsMatrix() {
  return (
    <section className="py-16 bg-white border-t border-slate-200" id="skills">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {SKILL_CATEGORIES.map((cat, idx) => (
            <div
              key={idx}
              className="clean-card p-6 rounded-2xl flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between pb-4 border-b border-slate-200 mb-5">
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    {idx === 0 && <Code2 className="w-4 h-4 text-blue-600" />}
                    {idx === 1 && <Database className="w-4 h-4 text-indigo-600" />}
                    {idx === 2 && <Wrench className="w-4 h-4 text-slate-700" />}
                    <span>{cat.title}</span>
                  </h3>
                </div>
              ...
              <div className="mt-5 pt-4 border-t border-slate-100 text-[11px] text-slate-500 flex items-center gap-1.5 font-medium">
