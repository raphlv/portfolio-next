      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-10 gap-5 sm:gap-6">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-600 mt-1.5 max-w-2xl font-normal">
          ...
          <div className="relative w-full md:w-72 shrink-0">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white text-slate-900 rounded-xl border border-slate-300 text-sm sm:text-xs focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600 placeholder:text-slate-400 font-medium shadow-sm"
            />
          </div>
        </div>

        {/* Category Tabs (Mobile Horizontal Swipe) */}
        <div className="flex items-center gap-2 mb-6 sm:mb-8 overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 no-scrollbar">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => setSelectedCategory(cat.id)}
              className={`px-3.5 sm:px-4 py-2 sm:py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-colors shrink-0 ${
        ...
        {displayedProjects.length === 0 && (
          <div className="clean-card rounded-2xl p-8 sm:p-12 text-center my-8">
            <p className="text-sm font-semibold text-slate-700">Tidak ada proyek yang sesuai dengan pencarian "{searchQuery}"</p>
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 mb-10">
          {displayedProjects.map((project) => (
            ...
            >
              ...
              {/* Bottom Actions */}
              <div className="p-4 sm:p-5 pt-0 flex items-center justify-between gap-2.5 text-xs">
                <button
                  onClick={() => setActiveModalProject(project)}
                  className="flex-1 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-900 font-bold transition-colors flex items-center justify-center gap-1.5"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="h-10 w-10 flex items-center justify-center rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 transition-colors shrink-0"
